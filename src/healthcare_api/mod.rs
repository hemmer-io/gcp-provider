...
impl<'a> Healthcare_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "data_mapper_workspace" => {
                self.plan_data_mapper_workspace(current_state, desired_input)
                    .await
            }
            "user_data_mapping" => {
                self.plan_user_data_mapping(current_state, desired_input)
                    .await
            }
            "location" => self.plan_location(current_state, desired_input).await,
            "consent_artifact" => {
                self.plan_consent_artifact(current_state, desired_input)
                    .await
            }
            "consent" => self.plan_consent(current_state, desired_input).await,
            "serie" => self.plan_serie(current_state, desired_input).await,
            "hl7_v2_store" => self.plan_hl7_v2_store(current_state, desired_input).await,
            "dicom_store" => self.plan_dicom_store(current_state, desired_input).await,
            "frame" => self.plan_frame(current_state, desired_input).await,
            "attribute_definition" => {
                self.plan_attribute_definition(current_state, desired_input)
                    .await
            }
            "instance" => self.plan_instance(current_state, desired_input).await,
            "bulkdata" => self.plan_bulkdata(current_state, desired_input).await,
            "consent_store" => self.plan_consent_store(current_state, desired_input).await,
            "message" => self.plan_message(current_state, desired_input).await,
            "dataset" => self.plan_dataset(current_state, desired_input).await,
            "fhir_store" => self.plan_fhir_store(current_state, desired_input).await,
            "nlp" => self.plan_nlp(current_state, desired_input).await,
            ...
            "fhir" => self.plan_fhir(current_state, desired_input).await,
            "annotation_store" => {
                self.plan_annotation_store(current_state, desired_input)
                    .await
            }
            "dataset" => self.plan_dataset(current_state, desired_input).await,
            "annotation" => self.plan_annotation(current_state, desired_input).await,
            "operation" => self.plan_operation(current_state, desired_input).await,
            "dicom_web" => self.plan_dicom_web(current_state, desired_input).await,
            "studie" => self.plan_studie(current_state, desired_input).await,
            "hl7_v2_store" => self.plan_hl7_v2_store(current_state, desired_input).await,
            "frame" => self.plan_frame(current_state, desired_input).await,
            "instance" => self.plan_instance(current_state, desired_input).await,
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "data_mapper_workspace" => self.create_data_mapper_workspace(input).await,
            "user_data_mapping" => self.create_user_data_mapping(input).await,
            "location" => self.create_location(input).await,
            "consent_artifact" => self.create_consent_artifact(input).await,
            "consent" => self.create_consent(input).await,
            "serie" => self.create_serie(input).await,
            "hl7_v2_store" => self.create_hl7_v2_store(input).await,
            "dicom_store" => self.create_dicom_store(input).await,
            "frame" => self.create_frame(input).await,
            "attribute_definition" => self.create_attribute_definition(input).await,
            "instance" => self.create_instance(input).await,
            "bulkdata" => self.create_bulkdata(input).await,
            "consent_store" => self.create_consent_store(input).await,
            "message" => self.create_message(input).await,
            "dataset" => self.create_dataset(input).await,
            "fhir_store" => self.create_fhir_store(input).await,
            "nlp" => self.create_nlp(input).await,
            ...
            "dicom_store" => self.create_dicom_store(input).await,
            "fhir" => self.create_fhir(input).await,
            "annotation_store" => self.create_annotation_store(input).await,
            "dataset" => self.create_dataset(input).await,
            "annotation" => self.create_annotation(input).await,
            "operation" => self.create_operation(input).await,
            "dicom_web" => self.create_dicom_web(input).await,
            "studie" => self.create_studie(input).await,
            "hl7_v2_store" => self.create_hl7_v2_store(input).await,
            "frame" => self.create_frame(input).await,
            "instance" => self.create_instance(input).await,
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "data_mapper_workspace" => self.read_data_mapper_workspace(id).await,
            "user_data_mapping" => self.read_user_data_mapping(id).await,
            "location" => self.read_location(id).await,
            "consent_artifact" => self.read_consent_artifact(id).await,
            "consent" => self.read_consent(id).await,
            "serie" => self.read_serie(id).await,
            "hl7_v2_store" => self.read_hl7_v2_store(id).await,
            "dicom_store" => self.read_dicom_store(id).await,
            "frame" => self.read_frame(id).await,
            "attribute_definition" => self.read_attribute_definition(id).await,
            "instance" => self.read_instance(id).await,
            "bulkdata" => self.read_bulkdata(id).await,
            "consent_store" => self.read_consent_store(id).await,
            "message" => self.read_message(id).await,
            "dataset" => self.read_dataset(id).await,
            "fhir_store" => self.read_fhir_store(id).await,
            "nlp" => self.read_nlp(id).await,
            ...
            "dicom_store" => self.read_dicom_store(id).await,
            "fhir" => self.read_fhir(id).await,
            "annotation_store" => self.read_annotation_store(id).await,
            "dataset" => self.read_dataset(id).await,
            "annotation" => self.read_annotation(id).await,
            "operation" => self.read_operation(id).await,
            "dicom_web" => self.read_dicom_web(id).await,
            "studie" => self.read_studie(id).await,
            "hl7_v2_store" => self.read_hl7_v2_store(id).await,
            "frame" => self.read_frame(id).await,
            "instance" => self.read_instance(id).await,
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "data_mapper_workspace" => self.update_data_mapper_workspace(id, input).await,
            "user_data_mapping" => self.update_user_data_mapping(id, input).await,
            "location" => self.update_location(id, input).await,
            "consent_artifact" => self.update_consent_artifact(id, input).await,
            "consent" => self.update_consent(id, input).await,
            "serie" => self.update_serie(id, input).await,
            "hl7_v2_store" => self.update_hl7_v2_store(id, input).await,
            "dicom_store" => self.update_dicom_store(id, input).await,
            "frame" => self.update_frame(id, input).await,
            "attribute_definition" => self.update_attribute_definition(id, input).await,
            "instance" => self.update_instance(id, input).await,
            "bulkdata" => self.update_bulkdata(id, input).await,
            "consent_store" => self.update_consent_store(id, input).await,
            "message" => self.update_message(id, input).await,
            "dataset" => self.update_dataset(id, input).await,
            "fhir_store" => self.update_fhir_store(id, input).await,
            "nlp" => self.update_nlp(id, input).await,
            ...
            "dicom_store" => self.update_dicom_store(id, input).await,
            "fhir" => self.update_fhir(id, input).await,
            "annotation_store" => self.update_annotation_store(id, input).await,
            "dataset" => self.update_dataset(id, input).await,
            "annotation" => self.update_annotation(id, input).await,
            "operation" => self.update_operation(id, input).await,
            "dicom_web" => self.update_dicom_web(id, input).await,
            "studie" => self.update_studie(id, input).await,
            "hl7_v2_store" => self.update_hl7_v2_store(id, input).await,
            "frame" => self.update_frame(id, input).await,
            "instance" => self.update_instance(id, input).await,
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "data_mapper_workspace" => self.delete_data_mapper_workspace(id).await,
            "user_data_mapping" => self.delete_user_data_mapping(id).await,
            "location" => self.delete_location(id).await,
            "consent_artifact" => self.delete_consent_artifact(id).await,
            "consent" => self.delete_consent(id).await,
            "serie" => self.delete_serie(id).await,
            "hl7_v2_store" => self.delete_hl7_v2_store(id).await,
            "dicom_store" => self.delete_dicom_store(id).await,
            "frame" => self.delete_frame(id).await,
            "attribute_definition" => self.delete_attribute_definition(id).await,
            "instance" => self.delete_instance(id).await,
            "bulkdata" => self.delete_bulkdata(id).await,
            "consent_store" => self.delete_consent_store(id).await,
            "message" => self.delete_message(id).await,
            "dataset" => self.delete_dataset(id).await,
            "fhir_store" => self.delete_fhir_store(id).await,
            "nlp" => self.delete_nlp(id).await,
            ...
            "dicom_store" => self.delete_dicom_store(id).await,
            "fhir" => self.delete_fhir(id).await,
            "annotation_store" => self.delete_annotation_store(id).await,
            "dataset" => self.delete_dataset(id).await,
            "annotation" => self.delete_annotation(id).await,
            "operation" => self.delete_operation(id).await,
            "dicom_web" => self.delete_dicom_web(id).await,
            "studie" => self.delete_studie(id).await,
            "hl7_v2_store" => self.delete_hl7_v2_store(id).await,
            "frame" => self.delete_frame(id).await,
            "instance" => self.delete_instance(id).await,
    ...
    async fn create_data_mapper_workspace(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a data_mapper_workspace resource
    async fn read_data_mapper_workspace(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_data_mapper_workspace(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_user_data_mapping(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a user_data_mapping resource
    async fn read_user_data_mapping(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_user_data_mapping(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_consent_artifact(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a consent_artifact resource
    async fn read_consent_artifact(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_consent_artifact(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_consent(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a consent resource
    async fn read_consent(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a consent resource
    async fn update_consent(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a consent resource
    async fn delete_consent(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_hl7_v2_store(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a hl7_v2_store resource
    async fn read_hl7_v2_store(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a hl7_v2_store resource
    async fn update_hl7_v2_store(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a hl7_v2_store resource
    async fn delete_hl7_v2_store(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_dicom_store(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a dicom_store resource
    async fn read_dicom_store(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a dicom_store resource
    async fn update_dicom_store(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a dicom_store resource
    async fn delete_dicom_store(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_frame(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a frame resource
    async fn read_frame(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a frame resource
    async fn update_frame(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a frame resource
    async fn delete_frame(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_attribute_definition(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a attribute_definition resource
    async fn read_attribute_definition(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_attribute_definition(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_bulkdata(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a bulkdata resource
    async fn read_bulkdata(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a bulkdata resource
    async fn update_bulkdata(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a bulkdata resource
    async fn delete_bulkdata(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_consent_store(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a consent_store resource
    async fn read_consent_store(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a consent_store resource
    async fn update_consent_store(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a consent_store resource
    async fn delete_consent_store(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_fhir_store(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a fhir_store resource
    async fn read_fhir_store(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a fhir_store resource
    async fn update_fhir_store(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a fhir_store resource
    async fn delete_fhir_store(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_nlp(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a nlp resource
    async fn read_nlp(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a nlp resource
    async fn update_nlp(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a nlp resource
    async fn delete_nlp(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_annotation_store(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a annotation_store resource
    async fn read_annotation_store(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_annotation_store(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_dicom_web(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a dicom_web resource
    async fn read_dicom_web(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a dicom_web resource
    async fn update_dicom_web(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a dicom_web resource
    async fn delete_dicom_web(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
