...
impl<'a> Monitoring_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "metrics_scope" => self.plan_metrics_scope(current_state, desired_input).await,
            ...
            "alert_policie" => self.plan_alert_policie(current_state, desired_input).await,
            ...
            "metric_descriptor" => {
                self.plan_metric_descriptor(current_state, desired_input)
                    .await
            }
            "member" => self.plan_member(current_state, desired_input).await,
            "service" => self.plan_service(current_state, desired_input).await,
            "uptime_check_config" => {
                self.plan_uptime_check_config(current_state, desired_input)
                    .await
            }
            "service_level_objective" => {
                self.plan_service_level_objective(current_state, desired_input)
                    .await
            }
            "snooze" => self.plan_snooze(current_state, desired_input).await,
            "notification_channel_descriptor" => {
                self.plan_notification_channel_descriptor(current_state, desired_input)
                    .await
            }
            "alert" => self.plan_alert(current_state, desired_input).await,
            "uptime_check_ip" => {
                self.plan_uptime_check_ip(current_state, desired_input)
                    .await
            }
            "collectd_time_serie" => {
                self.plan_collectd_time_serie(current_state, desired_input)
                    .await
            }
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "monitoring_api", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "metrics_scope" => self.create_metrics_scope(input).await,
            "time_serie" => self.create_time_serie(input).await,
            "notification_channel" => self.create_notification_channel(input).await,
            "alert_policie" => self.create_alert_policie(input).await,
            "group" => self.create_group(input).await,
            "monitored_resource_descriptor" => {
                self.create_monitored_resource_descriptor(input).await
            }
            "metric_descriptor" => self.create_metric_descriptor(input).await,
            "member" => self.create_member(input).await,
            "service" => self.create_service(input).await,
            "uptime_check_config" => self.create_uptime_check_config(input).await,
            "service_level_objective" => self.create_service_level_objective(input).await,
            "snooze" => self.create_snooze(input).await,
            "notification_channel_descriptor" => {
                self.create_notification_channel_descriptor(input).await
            }
            "alert" => self.create_alert(input).await,
            "uptime_check_ip" => self.create_uptime_check_ip(input).await,
            "collectd_time_serie" => self.create_collectd_time_serie(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "monitoring_api", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "metrics_scope" => self.read_metrics_scope(id).await,
            "time_serie" => self.read_time_serie(id).await,
            "notification_channel" => self.read_notification_channel(id).await,
            "alert_policie" => self.read_alert_policie(id).await,
            "group" => self.read_group(id).await,
            "monitored_resource_descriptor" => self.read_monitored_resource_descriptor(id).await,
            "metric_descriptor" => self.read_metric_descriptor(id).await,
            "member" => self.read_member(id).await,
            "service" => self.read_service(id).await,
            "uptime_check_config" => self.read_uptime_check_config(id).await,
            "service_level_objective" => self.read_service_level_objective(id).await,
            "snooze" => self.read_snooze(id).await,
            "notification_channel_descriptor" => {
                self.read_notification_channel_descriptor(id).await
            }
            "alert" => self.read_alert(id).await,
            "uptime_check_ip" => self.read_uptime_check_ip(id).await,
            "collectd_time_serie" => self.read_collectd_time_serie(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "monitoring_api", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "metrics_scope" => self.update_metrics_scope(id, input).await,
            "time_serie" => self.update_time_serie(id, input).await,
            "notification_channel" => self.update_notification_channel(id, input).await,
            "alert_policie" => self.update_alert_policie(id, input).await,
            "group" => self.update_group(id, input).await,
            "monitored_resource_descriptor" => {
                self.update_monitored_resource_descriptor(id, input).await
            }
            "metric_descriptor" => self.update_metric_descriptor(id, input).await,
            "member" => self.update_member(id, input).await,
            "service" => self.update_service(id, input).await,
            "uptime_check_config" => self.update_uptime_check_config(id, input).await,
            "service_level_objective" => self.update_service_level_objective(id, input).await,
            "snooze" => self.update_snooze(id, input).await,
            "notification_channel_descriptor" => {
                self.update_notification_channel_descriptor(id, input).await
            }
            "alert" => self.update_alert(id, input).await,
            "uptime_check_ip" => self.update_uptime_check_ip(id, input).await,
            "collectd_time_serie" => self.update_collectd_time_serie(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "monitoring_api", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "metrics_scope" => self.delete_metrics_scope(id).await,
            "time_serie" => self.delete_time_serie(id).await,
            "notification_channel" => self.delete_notification_channel(id).await,
            "alert_policie" => self.delete_alert_policie(id).await,
            "group" => self.delete_group(id).await,
            "monitored_resource_descriptor" => self.delete_monitored_resource_descriptor(id).await,
            "metric_descriptor" => self.delete_metric_descriptor(id).await,
            "member" => self.delete_member(id).await,
            "service" => self.delete_service(id).await,
            "uptime_check_config" => self.delete_uptime_check_config(id).await,
            "service_level_objective" => self.delete_service_level_objective(id).await,
            "snooze" => self.delete_snooze(id).await,
            "notification_channel_descriptor" => {
                self.delete_notification_channel_descriptor(id).await
            }
            "alert" => self.delete_alert(id).await,
            "uptime_check_ip" => self.delete_uptime_check_ip(id).await,
            "collectd_time_serie" => self.delete_collectd_time_serie(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "monitoring_api", resource_name
            ))),
    ...
    async fn create_metrics_scope(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a metrics_scope resource
    async fn read_metrics_scope(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a metrics_scope resource
    async fn update_metrics_scope(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a metrics_scope resource
    async fn delete_metrics_scope(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_alert_policie(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a alert_policie resource
    async fn read_alert_policie(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a alert_policie resource
    async fn update_alert_policie(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a alert_policie resource
    async fn delete_alert_policie(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_metric_descriptor(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a metric_descriptor resource
    async fn read_metric_descriptor(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_metric_descriptor(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_uptime_check_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a uptime_check_config resource
    async fn read_uptime_check_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_uptime_check_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_service_level_objective(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a service_level_objective resource
    async fn read_service_level_objective(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_service_level_objective(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_snooze(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a snooze resource
    async fn read_snooze(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a snooze resource
    async fn update_snooze(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a snooze resource
    async fn delete_snooze(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn read_notification_channel_descriptor(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_notification_channel_descriptor(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_uptime_check_ip(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a uptime_check_ip resource
    async fn read_uptime_check_ip(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_uptime_check_ip(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_collectd_time_serie(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a collectd_time_serie resource
    async fn read_collectd_time_serie(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_collectd_time_serie(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
}
