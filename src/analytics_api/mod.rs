...
impl<'a> Analytics_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "mcf" => self.plan_mcf(current_state, desired_input).await,
            "account_user_link" => {
                self.plan_account_user_link(current_state, desired_input)
                    .await
            }
            "realtime" => self.plan_realtime(current_state, desired_input).await,
            "filter" => self.plan_filter(current_state, desired_input).await,
            "column" => self.plan_column(current_state, desired_input).await,
            "remarketing_audience" => {
                self.plan_remarketing_audience(current_state, desired_input)
                    .await
            }
            ...
            "webpropertie" => self.plan_webpropertie(current_state, desired_input).await,
            "goal" => self.plan_goal(current_state, desired_input).await,
            "client_id" => self.plan_client_id(current_state, desired_input).await,
            "unsampled_report" => {
                self.plan_unsampled_report(current_state, desired_input)
                    .await
            }
            "profile" => self.plan_profile(current_state, desired_input).await,
            "custom_data_source" => {
                self.plan_custom_data_source(current_state, desired_input)
                    .await
            }
            "ga" => self.plan_ga(current_state, desired_input).await,
            "experiment" => self.plan_experiment(current_state, desired_input).await,
            "web_property_ad_words_link" => {
                self.plan_web_property_ad_words_link(current_state, desired_input)
                    .await
            }
            "webproperty_user_link" => {
                self.plan_webproperty_user_link(current_state, desired_input)
                    .await
            }
            "profile_filter_link" => {
                self.plan_profile_filter_link(current_state, desired_input)
                    .await
            }
            "segment" => self.plan_segment(current_state, desired_input).await,
            "provisioning" => self.plan_provisioning(current_state, desired_input).await,
            "account_summarie" => {
                self.plan_account_summarie(current_state, desired_input)
                    .await
            }
            "profile_user_link" => {
                self.plan_profile_user_link(current_state, desired_input)
                    .await
            }
            "custom_dimension" => {
            ...
            "user_deletion_request" => {
                self.plan_user_deletion_request(current_state, desired_input)
                    .await
            }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "mcf" => self.create_mcf(input).await,
            "account_user_link" => self.create_account_user_link(input).await,
            "realtime" => self.create_realtime(input).await,
            "filter" => self.create_filter(input).await,
            "column" => self.create_column(input).await,
            "remarketing_audience" => self.create_remarketing_audience(input).await,
            "account" => self.create_account(input).await,
            "custom_metric" => self.create_custom_metric(input).await,
            "webpropertie" => self.create_webpropertie(input).await,
            "goal" => self.create_goal(input).await,
            "client_id" => self.create_client_id(input).await,
            "unsampled_report" => self.create_unsampled_report(input).await,
            "profile" => self.create_profile(input).await,
            "custom_data_source" => self.create_custom_data_source(input).await,
            "ga" => self.create_ga(input).await,
            "experiment" => self.create_experiment(input).await,
            "web_property_ad_words_link" => self.create_web_property_ad_words_link(input).await,
            "webproperty_user_link" => self.create_webproperty_user_link(input).await,
            "profile_filter_link" => self.create_profile_filter_link(input).await,
            "segment" => self.create_segment(input).await,
            "provisioning" => self.create_provisioning(input).await,
            "account_summarie" => self.create_account_summarie(input).await,
            "profile_user_link" => self.create_profile_user_link(input).await,
            "custom_dimension" => self.create_custom_dimension(input).await,
            "upload" => self.create_upload(input).await,
            "user_deletion_request" => self.create_user_deletion_request(input).await,
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "mcf" => self.read_mcf(id).await,
            "account_user_link" => self.read_account_user_link(id).await,
            "realtime" => self.read_realtime(id).await,
            "filter" => self.read_filter(id).await,
            "column" => self.read_column(id).await,
            "remarketing_audience" => self.read_remarketing_audience(id).await,
            "account" => self.read_account(id).await,
            "custom_metric" => self.read_custom_metric(id).await,
            "webpropertie" => self.read_webpropertie(id).await,
            "goal" => self.read_goal(id).await,
            "client_id" => self.read_client_id(id).await,
            "unsampled_report" => self.read_unsampled_report(id).await,
            "profile" => self.read_profile(id).await,
            "custom_data_source" => self.read_custom_data_source(id).await,
            "ga" => self.read_ga(id).await,
            "experiment" => self.read_experiment(id).await,
            "web_property_ad_words_link" => self.read_web_property_ad_words_link(id).await,
            "webproperty_user_link" => self.read_webproperty_user_link(id).await,
            "profile_filter_link" => self.read_profile_filter_link(id).await,
            "segment" => self.read_segment(id).await,
            "provisioning" => self.read_provisioning(id).await,
            "account_summarie" => self.read_account_summarie(id).await,
            "profile_user_link" => self.read_profile_user_link(id).await,
            "custom_dimension" => self.read_custom_dimension(id).await,
            "upload" => self.read_upload(id).await,
            "user_deletion_request" => self.read_user_deletion_request(id).await,
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "mcf" => self.update_mcf(id, input).await,
            "account_user_link" => self.update_account_user_link(id, input).await,
            "realtime" => self.update_realtime(id, input).await,
            "filter" => self.update_filter(id, input).await,
            "column" => self.update_column(id, input).await,
            "remarketing_audience" => self.update_remarketing_audience(id, input).await,
            "account" => self.update_account(id, input).await,
            "custom_metric" => self.update_custom_metric(id, input).await,
            "webpropertie" => self.update_webpropertie(id, input).await,
            "goal" => self.update_goal(id, input).await,
            "client_id" => self.update_client_id(id, input).await,
            "unsampled_report" => self.update_unsampled_report(id, input).await,
            "profile" => self.update_profile(id, input).await,
            "custom_data_source" => self.update_custom_data_source(id, input).await,
            "ga" => self.update_ga(id, input).await,
            "experiment" => self.update_experiment(id, input).await,
            "web_property_ad_words_link" => self.update_web_property_ad_words_link(id, input).await,
            "webproperty_user_link" => self.update_webproperty_user_link(id, input).await,
            "profile_filter_link" => self.update_profile_filter_link(id, input).await,
            "segment" => self.update_segment(id, input).await,
            "provisioning" => self.update_provisioning(id, input).await,
            "account_summarie" => self.update_account_summarie(id, input).await,
            "profile_user_link" => self.update_profile_user_link(id, input).await,
            "custom_dimension" => self.update_custom_dimension(id, input).await,
            "upload" => self.update_upload(id, input).await,
            "user_deletion_request" => self.update_user_deletion_request(id, input).await,
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "mcf" => self.delete_mcf(id).await,
            "account_user_link" => self.delete_account_user_link(id).await,
            "realtime" => self.delete_realtime(id).await,
            "filter" => self.delete_filter(id).await,
            "column" => self.delete_column(id).await,
            "remarketing_audience" => self.delete_remarketing_audience(id).await,
            "account" => self.delete_account(id).await,
            "custom_metric" => self.delete_custom_metric(id).await,
            "webpropertie" => self.delete_webpropertie(id).await,
            "goal" => self.delete_goal(id).await,
            "client_id" => self.delete_client_id(id).await,
            "unsampled_report" => self.delete_unsampled_report(id).await,
            "profile" => self.delete_profile(id).await,
            "custom_data_source" => self.delete_custom_data_source(id).await,
            "ga" => self.delete_ga(id).await,
            "experiment" => self.delete_experiment(id).await,
            "web_property_ad_words_link" => self.delete_web_property_ad_words_link(id).await,
            "webproperty_user_link" => self.delete_webproperty_user_link(id).await,
            "profile_filter_link" => self.delete_profile_filter_link(id).await,
            "segment" => self.delete_segment(id).await,
            "provisioning" => self.delete_provisioning(id).await,
            "account_summarie" => self.delete_account_summarie(id).await,
            "profile_user_link" => self.delete_profile_user_link(id).await,
            "custom_dimension" => self.delete_custom_dimension(id).await,
            "upload" => self.delete_upload(id).await,
            "user_deletion_request" => self.delete_user_deletion_request(id).await,
    ...
    async fn create_mcf(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a mcf resource
    async fn read_mcf(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a mcf resource
    async fn update_mcf(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a mcf resource
    async fn delete_mcf(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_account_user_link(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a account_user_link resource
    async fn read_account_user_link(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_account_user_link(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_realtime(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a realtime resource
    async fn read_realtime(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a realtime resource
    async fn update_realtime(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a realtime resource
    async fn delete_realtime(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_column(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a column resource
    async fn read_column(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a column resource
    async fn update_column(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn create_remarketing_audience(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a remarketing_audience resource
    async fn read_remarketing_audience(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_remarketing_audience(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_webpropertie(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a webpropertie resource
    async fn read_webpropertie(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a webpropertie resource
    async fn update_webpropertie(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a webpropertie resource
    async fn delete_webpropertie(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_goal(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a goal resource
    async fn read_goal(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a goal resource
    async fn update_goal(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn create_client_id(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a client_id resource
    async fn read_client_id(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a client_id resource
    async fn update_client_id(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a client_id resource
    async fn delete_client_id(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_unsampled_report(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a unsampled_report resource
    async fn read_unsampled_report(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_unsampled_report(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_custom_data_source(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a custom_data_source resource
    async fn read_custom_data_source(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_custom_data_source(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_ga(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a ga resource
    async fn read_ga(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a ga resource
    async fn update_ga(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a ga resource
    async fn delete_ga(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn read_web_property_ad_words_link(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_web_property_ad_words_link(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_webproperty_user_link(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a webproperty_user_link resource
    async fn read_webproperty_user_link(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_webproperty_user_link(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_profile_filter_link(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a profile_filter_link resource
    async fn read_profile_filter_link(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_profile_filter_link(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_provisioning(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a provisioning resource
    async fn read_provisioning(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a provisioning resource
    async fn update_provisioning(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a provisioning resource
    async fn delete_provisioning(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_profile_user_link(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a profile_user_link resource
    async fn read_profile_user_link(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_profile_user_link(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_user_deletion_request(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a user_deletion_request resource
    async fn read_user_deletion_request(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_user_deletion_request(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
