...
impl<'a> Firebaseappcheck_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "debug_token" => self.plan_debug_token(current_state, desired_input).await,
            "app_attest_config" => {
                self.plan_app_attest_config(current_state, desired_input)
                    .await
            }
            "device_check_config" => {
                self.plan_device_check_config(current_state, desired_input)
                    .await
            }
            "recaptcha_enterprise_config" => {
                self.plan_recaptcha_enterprise_config(current_state, desired_input)
                    .await
            }
            "oauth_client" => self.plan_oauth_client(current_state, desired_input).await,
            "jwk" => self.plan_jwk(current_state, desired_input).await,
            "app" => self.plan_app(current_state, desired_input).await,
            "recaptcha_v3_config" => {
                self.plan_recaptcha_v3_config(current_state, desired_input)
                    .await
            }
            "service" => self.plan_service(current_state, desired_input).await,
            "safety_net_config" => {
                self.plan_safety_net_config(current_state, desired_input)
                    .await
            }
            "resource_policie" => {
            ...
            "play_integrity_config" => {
                self.plan_play_integrity_config(current_state, desired_input)
                    .await
            }
            "resource_policie" => {
                self.plan_resource_policie(current_state, desired_input)
            ...
            "app" => self.plan_app(current_state, desired_input).await,
            "recaptcha_config" => {
                self.plan_recaptcha_config(current_state, desired_input)
                    .await
            }
            ...
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                ...
                "firebaseappcheck_api", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "debug_token" => self.create_debug_token(input).await,
            "app_attest_config" => self.create_app_attest_config(input).await,
            "device_check_config" => self.create_device_check_config(input).await,
            "recaptcha_enterprise_config" => self.create_recaptcha_enterprise_config(input).await,
            "oauth_client" => self.create_oauth_client(input).await,
            "jwk" => self.create_jwk(input).await,
            "app" => self.create_app(input).await,
            "recaptcha_v3_config" => self.create_recaptcha_v3_config(input).await,
            "service" => self.create_service(input).await,
            "safety_net_config" => self.create_safety_net_config(input).await,
            "resource_policie" => self.create_resource_policie(input).await,
            "play_integrity_config" => self.create_play_integrity_config(input).await,
            "resource_policie" => self.create_resource_policie(input).await,
            "oauth_client" => self.create_oauth_client(input).await,
            "jwk" => self.create_jwk(input).await,
            "app" => self.create_app(input).await,
            "recaptcha_config" => self.create_recaptcha_config(input).await,
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "debug_token" => self.read_debug_token(id).await,
            "app_attest_config" => self.read_app_attest_config(id).await,
            "device_check_config" => self.read_device_check_config(id).await,
            "recaptcha_enterprise_config" => self.read_recaptcha_enterprise_config(id).await,
            "oauth_client" => self.read_oauth_client(id).await,
            "jwk" => self.read_jwk(id).await,
            "app" => self.read_app(id).await,
            "recaptcha_v3_config" => self.read_recaptcha_v3_config(id).await,
            "service" => self.read_service(id).await,
            "safety_net_config" => self.read_safety_net_config(id).await,
            "resource_policie" => self.read_resource_policie(id).await,
            "play_integrity_config" => self.read_play_integrity_config(id).await,
            "resource_policie" => self.read_resource_policie(id).await,
            "oauth_client" => self.read_oauth_client(id).await,
            "jwk" => self.read_jwk(id).await,
            "app" => self.read_app(id).await,
            "recaptcha_config" => self.read_recaptcha_config(id).await,
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "debug_token" => self.update_debug_token(id, input).await,
            "app_attest_config" => self.update_app_attest_config(id, input).await,
            "device_check_config" => self.update_device_check_config(id, input).await,
            "recaptcha_enterprise_config" => {
                self.update_recaptcha_enterprise_config(id, input).await
            }
            "oauth_client" => self.update_oauth_client(id, input).await,
            "jwk" => self.update_jwk(id, input).await,
            "app" => self.update_app(id, input).await,
            "recaptcha_v3_config" => self.update_recaptcha_v3_config(id, input).await,
            "service" => self.update_service(id, input).await,
            "safety_net_config" => self.update_safety_net_config(id, input).await,
            "resource_policie" => self.update_resource_policie(id, input).await,
            "play_integrity_config" => self.update_play_integrity_config(id, input).await,
            "resource_policie" => self.update_resource_policie(id, input).await,
            "oauth_client" => self.update_oauth_client(id, input).await,
            "jwk" => self.update_jwk(id, input).await,
            "app" => self.update_app(id, input).await,
            "recaptcha_config" => self.update_recaptcha_config(id, input).await,
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "debug_token" => self.delete_debug_token(id).await,
            "app_attest_config" => self.delete_app_attest_config(id).await,
            "device_check_config" => self.delete_device_check_config(id).await,
            "recaptcha_enterprise_config" => self.delete_recaptcha_enterprise_config(id).await,
            "oauth_client" => self.delete_oauth_client(id).await,
            "jwk" => self.delete_jwk(id).await,
            "app" => self.delete_app(id).await,
            "recaptcha_v3_config" => self.delete_recaptcha_v3_config(id).await,
            "service" => self.delete_service(id).await,
            "safety_net_config" => self.delete_safety_net_config(id).await,
            "resource_policie" => self.delete_resource_policie(id).await,
            "play_integrity_config" => self.delete_play_integrity_config(id).await,
            "resource_policie" => self.delete_resource_policie(id).await,
            "oauth_client" => self.delete_oauth_client(id).await,
            "jwk" => self.delete_jwk(id).await,
            "app" => self.delete_app(id).await,
            "recaptcha_config" => self.delete_recaptcha_config(id).await,
    ...
    async fn create_debug_token(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a debug_token resource
    async fn read_debug_token(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a debug_token resource
    async fn update_debug_token(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a debug_token resource
    async fn delete_debug_token(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_app_attest_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a app_attest_config resource
    async fn read_app_attest_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_app_attest_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_device_check_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a device_check_config resource
    async fn read_device_check_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_device_check_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn read_recaptcha_enterprise_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_recaptcha_enterprise_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_oauth_client(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a oauth_client resource
    async fn read_oauth_client(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a oauth_client resource
    async fn update_oauth_client(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn create_recaptcha_v3_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a recaptcha_v3_config resource
    async fn read_recaptcha_v3_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_recaptcha_v3_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_safety_net_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a safety_net_config resource
    async fn read_safety_net_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_safety_net_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_play_integrity_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a play_integrity_config resource
    async fn read_play_integrity_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_play_integrity_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_recaptcha_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a recaptcha_config resource
    async fn read_recaptcha_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_recaptcha_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
