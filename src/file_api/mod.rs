//! File_api service for Gcp provider
//!
//! This module handles all file_api resources and their CRUD operations.

use hemmer_core::Result;
use hemmer_provider::{ResourceInput, ResourceOutput, ResourcePlan};

/// File_api service handler
...
impl<'a> File_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "snapshot" => self.plan_snapshot(current_state, desired_input).await,
            "location" => self.plan_location(current_state, desired_input).await,
            "share" => self.plan_share(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "file_api", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "snapshot" => self.create_snapshot(input).await,
            "location" => self.create_location(input).await,
            "share" => self.create_share(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "file_api", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "snapshot" => self.read_snapshot(id).await,
            "location" => self.read_location(id).await,
            "share" => self.read_share(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "file_api", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "snapshot" => self.update_snapshot(id, input).await,
            "location" => self.update_location(id, input).await,
            "share" => self.update_share(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "file_api", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "snapshot" => self.delete_snapshot(id).await,
            "location" => self.delete_location(id).await,
            "share" => self.delete_share(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "file_api", resource_name
            ))),
    ...
    /// Plan changes to a share resource
    async fn plan_share(
        &self,
        current_state: Option<&ResourceOutput>,
        desired_input: &ResourceInput,
    ...
    async fn create_share(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a share resource
    async fn read_share(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a share resource
    async fn update_share(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
