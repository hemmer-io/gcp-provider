...
impl<'a> Tagmanager_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "reauthorize_environment" => {
                self.plan_reauthorize_environment(current_state, desired_input)
                    .await
            }
            "folder" => self.plan_folder(current_state, desired_input).await,
            "version" => self.plan_version(current_state, desired_input).await,
            "move_folder" => self.plan_move_folder(current_state, desired_input).await,
            "account" => self.plan_account(current_state, desired_input).await,
            "trigger" => self.plan_trigger(current_state, desired_input).await,
            "tag" => self.plan_tag(current_state, desired_input).await,
            "user_permission" => {
                self.plan_user_permission(current_state, desired_input)
                    .await
            }
            "zone" => self.plan_zone(current_state, desired_input).await,
            "version_header" => self.plan_version_header(current_state, desired_input).await,
            "version" => self.plan_version(current_state, desired_input).await,
            "environment" => self.plan_environment(current_state, desired_input).await,
            "destination" => self.plan_destination(current_state, desired_input).await,
            "trigger" => self.plan_trigger(current_state, desired_input).await,
            "transformation" => self.plan_transformation(current_state, desired_input).await,
            "workspace" => self.plan_workspace(current_state, desired_input).await,
            "gtag_config" => self.plan_gtag_config(current_state, desired_input).await,
            "container" => self.plan_container(current_state, desired_input).await,
            "built_in_variable" => {
                self.plan_built_in_variable(current_state, desired_input)
                    .await
            }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "reauthorize_environment" => self.create_reauthorize_environment(input).await,
            "folder" => self.create_folder(input).await,
            "version" => self.create_version(input).await,
            "move_folder" => self.create_move_folder(input).await,
            "account" => self.create_account(input).await,
            "trigger" => self.create_trigger(input).await,
            "tag" => self.create_tag(input).await,
            "user_permission" => self.create_user_permission(input).await,
            "zone" => self.create_zone(input).await,
            "version_header" => self.create_version_header(input).await,
            "version" => self.create_version(input).await,
            "environment" => self.create_environment(input).await,
            "destination" => self.create_destination(input).await,
            "trigger" => self.create_trigger(input).await,
            "transformation" => self.create_transformation(input).await,
            "workspace" => self.create_workspace(input).await,
            "gtag_config" => self.create_gtag_config(input).await,
            "container" => self.create_container(input).await,
            "built_in_variable" => self.create_built_in_variable(input).await,
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "reauthorize_environment" => self.read_reauthorize_environment(id).await,
            "folder" => self.read_folder(id).await,
            "version" => self.read_version(id).await,
            "move_folder" => self.read_move_folder(id).await,
            "account" => self.read_account(id).await,
            "trigger" => self.read_trigger(id).await,
            "tag" => self.read_tag(id).await,
            "user_permission" => self.read_user_permission(id).await,
            "zone" => self.read_zone(id).await,
            "version_header" => self.read_version_header(id).await,
            "version" => self.read_version(id).await,
            "environment" => self.read_environment(id).await,
            "destination" => self.read_destination(id).await,
            "trigger" => self.read_trigger(id).await,
            "transformation" => self.read_transformation(id).await,
            "workspace" => self.read_workspace(id).await,
            "gtag_config" => self.read_gtag_config(id).await,
            "container" => self.read_container(id).await,
            "built_in_variable" => self.read_built_in_variable(id).await,
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "reauthorize_environment" => self.update_reauthorize_environment(id, input).await,
            "folder" => self.update_folder(id, input).await,
            "version" => self.update_version(id, input).await,
            "move_folder" => self.update_move_folder(id, input).await,
            "account" => self.update_account(id, input).await,
            "trigger" => self.update_trigger(id, input).await,
            "tag" => self.update_tag(id, input).await,
            "user_permission" => self.update_user_permission(id, input).await,
            "zone" => self.update_zone(id, input).await,
            "version_header" => self.update_version_header(id, input).await,
            "version" => self.update_version(id, input).await,
            "environment" => self.update_environment(id, input).await,
            "destination" => self.update_destination(id, input).await,
            "trigger" => self.update_trigger(id, input).await,
            "transformation" => self.update_transformation(id, input).await,
            "workspace" => self.update_workspace(id, input).await,
            "gtag_config" => self.update_gtag_config(id, input).await,
            "container" => self.update_container(id, input).await,
            "built_in_variable" => self.update_built_in_variable(id, input).await,
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "reauthorize_environment" => self.delete_reauthorize_environment(id).await,
            "folder" => self.delete_folder(id).await,
            "version" => self.delete_version(id).await,
            "move_folder" => self.delete_move_folder(id).await,
            "account" => self.delete_account(id).await,
            "trigger" => self.delete_trigger(id).await,
            "tag" => self.delete_tag(id).await,
            "user_permission" => self.delete_user_permission(id).await,
            "zone" => self.delete_zone(id).await,
            "version_header" => self.delete_version_header(id).await,
            "version" => self.delete_version(id).await,
            "environment" => self.delete_environment(id).await,
            "destination" => self.delete_destination(id).await,
            "trigger" => self.delete_trigger(id).await,
            "transformation" => self.delete_transformation(id).await,
            "workspace" => self.delete_workspace(id).await,
            "gtag_config" => self.delete_gtag_config(id).await,
            "container" => self.delete_container(id).await,
            "built_in_variable" => self.delete_built_in_variable(id).await,
    ...
    async fn create_reauthorize_environment(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a reauthorize_environment resource
    async fn read_reauthorize_environment(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_reauthorize_environment(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_move_folder(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a move_folder resource
    async fn read_move_folder(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a move_folder resource
    async fn update_move_folder(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a move_folder resource
    async fn delete_move_folder(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_user_permission(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a user_permission resource
    async fn read_user_permission(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_user_permission(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_version_header(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a version_header resource
    async fn read_version_header(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_version_header(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_transformation(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a transformation resource
    async fn read_transformation(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_transformation(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_gtag_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a gtag_config resource
    async fn read_gtag_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a gtag_config resource
    async fn update_gtag_config(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a gtag_config resource
    async fn delete_gtag_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_built_in_variable(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a built_in_variable resource
    async fn read_built_in_variable(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_built_in_variable(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
