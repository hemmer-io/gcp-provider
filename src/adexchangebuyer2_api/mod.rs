...
impl<'a> Adexchangebuyer2_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "finalized_proposal" => {
                self.plan_finalized_proposal(current_state, desired_input)
                    .await
            }
            ...
            "non_billable_winning_bid" => {
                self.plan_non_billable_winning_bid(current_state, desired_input)
                    .await
            }
            "filter_set" => self.plan_filter_set(current_state, desired_input).await,
            ...
            "filtered_bid_request" => {
                self.plan_filtered_bid_request(current_state, desired_input)
                    .await
            }
            "losing_bid" => self.plan_losing_bid(current_state, desired_input).await,
            "impression_metric" => {
                self.plan_impression_metric(current_state, desired_input)
                    .await
            }
            "invitation" => self.plan_invitation(current_state, desired_input).await,
            "bid_responses_without_bid" => {
                self.plan_bid_responses_without_bid(current_state, desired_input)
                    .await
            }
            "product" => self.plan_product(current_state, desired_input).await,
            "client" => self.plan_client(current_state, desired_input).await,
            "bid_response_error" => {
                self.plan_bid_response_error(current_state, desired_input)
                    .await
            }
            "bid_metric" => self.plan_bid_metric(current_state, desired_input).await,
            "deal_association" => {
                self.plan_deal_association(current_state, desired_input)
                    .await
            }
            "filtered_bid" => self.plan_filtered_bid(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "adexchangebuyer2_api", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "finalized_proposal" => self.create_finalized_proposal(input).await,
            ...
            "non_billable_winning_bid" => self.create_non_billable_winning_bid(input).await,
            "filter_set" => self.create_filter_set(input).await,
            "creative" => self.create_creative(input).await,
            "user" => self.create_user(input).await,
            "filtered_bid_request" => self.create_filtered_bid_request(input).await,
            "losing_bid" => self.create_losing_bid(input).await,
            "impression_metric" => self.create_impression_metric(input).await,
            "invitation" => self.create_invitation(input).await,
            "bid_responses_without_bid" => self.create_bid_responses_without_bid(input).await,
            "product" => self.create_product(input).await,
            "client" => self.create_client(input).await,
            "bid_response_error" => self.create_bid_response_error(input).await,
            "bid_metric" => self.create_bid_metric(input).await,
            "deal_association" => self.create_deal_association(input).await,
            "filtered_bid" => self.create_filtered_bid(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "adexchangebuyer2_api", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "finalized_proposal" => self.read_finalized_proposal(id).await,
            ...
            "non_billable_winning_bid" => self.read_non_billable_winning_bid(id).await,
            "filter_set" => self.read_filter_set(id).await,
            "creative" => self.read_creative(id).await,
            "user" => self.read_user(id).await,
            "filtered_bid_request" => self.read_filtered_bid_request(id).await,
            "losing_bid" => self.read_losing_bid(id).await,
            "impression_metric" => self.read_impression_metric(id).await,
            "invitation" => self.read_invitation(id).await,
            "bid_responses_without_bid" => self.read_bid_responses_without_bid(id).await,
            "product" => self.read_product(id).await,
            "client" => self.read_client(id).await,
            "bid_response_error" => self.read_bid_response_error(id).await,
            "bid_metric" => self.read_bid_metric(id).await,
            "deal_association" => self.read_deal_association(id).await,
            "filtered_bid" => self.read_filtered_bid(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "adexchangebuyer2_api", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "finalized_proposal" => self.update_finalized_proposal(id, input).await,
            ...
            "non_billable_winning_bid" => self.update_non_billable_winning_bid(id, input).await,
            "filter_set" => self.update_filter_set(id, input).await,
            "creative" => self.update_creative(id, input).await,
            "user" => self.update_user(id, input).await,
            "filtered_bid_request" => self.update_filtered_bid_request(id, input).await,
            "losing_bid" => self.update_losing_bid(id, input).await,
            "impression_metric" => self.update_impression_metric(id, input).await,
            "invitation" => self.update_invitation(id, input).await,
            "bid_responses_without_bid" => self.update_bid_responses_without_bid(id, input).await,
            "product" => self.update_product(id, input).await,
            "client" => self.update_client(id, input).await,
            "bid_response_error" => self.update_bid_response_error(id, input).await,
            "bid_metric" => self.update_bid_metric(id, input).await,
            "deal_association" => self.update_deal_association(id, input).await,
            "filtered_bid" => self.update_filtered_bid(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "adexchangebuyer2_api", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "finalized_proposal" => self.delete_finalized_proposal(id).await,
            ...
            "non_billable_winning_bid" => self.delete_non_billable_winning_bid(id).await,
            "filter_set" => self.delete_filter_set(id).await,
            "creative" => self.delete_creative(id).await,
            "user" => self.delete_user(id).await,
            "filtered_bid_request" => self.delete_filtered_bid_request(id).await,
            "losing_bid" => self.delete_losing_bid(id).await,
            "impression_metric" => self.delete_impression_metric(id).await,
            "invitation" => self.delete_invitation(id).await,
            "bid_responses_without_bid" => self.delete_bid_responses_without_bid(id).await,
            "product" => self.delete_product(id).await,
            "client" => self.delete_client(id).await,
            "bid_response_error" => self.delete_bid_response_error(id).await,
            "bid_metric" => self.delete_bid_metric(id).await,
            "deal_association" => self.delete_deal_association(id).await,
            "filtered_bid" => self.delete_filtered_bid(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "adexchangebuyer2_api", resource_name
            ))),
    ...
    async fn create_finalized_proposal(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a finalized_proposal resource
    async fn read_finalized_proposal(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
    ...
    async fn delete_finalized_proposal(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn read_non_billable_winning_bid(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_non_billable_winning_bid(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_filter_set(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a filter_set resource
    async fn read_filter_set(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a filter_set resource
    async fn update_filter_set(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a filter_set resource
    async fn delete_filter_set(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_filtered_bid_request(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a filtered_bid_request resource
    async fn read_filtered_bid_request(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_filtered_bid_request(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_losing_bid(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a losing_bid resource
    async fn read_losing_bid(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a losing_bid resource
    async fn update_losing_bid(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a losing_bid resource
    async fn delete_losing_bid(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_impression_metric(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a impression_metric resource
    async fn read_impression_metric(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_impression_metric(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn read_bid_responses_without_bid(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_bid_responses_without_bid(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_bid_response_error(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a bid_response_error resource
    async fn read_bid_response_error(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_bid_response_error(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_bid_metric(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a bid_metric resource
    async fn read_bid_metric(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a bid_metric resource
    async fn update_bid_metric(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a bid_metric resource
    async fn delete_bid_metric(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_deal_association(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a deal_association resource
    async fn read_deal_association(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_deal_association(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_filtered_bid(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a filtered_bid resource
    async fn read_filtered_bid(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a filtered_bid resource
    async fn update_filtered_bid(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a filtered_bid resource
    async fn delete_filtered_bid(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
}
