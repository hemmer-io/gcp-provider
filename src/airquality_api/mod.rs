...
impl<'a> Airquality_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "current_condition" => {
                self.plan_current_condition(current_state, desired_input)
                    .await
            }
            "history" => self.plan_history(current_state, desired_input).await,
            "forecast" => self.plan_forecast(current_state, desired_input).await,
            "heatmap_tile" => self.plan_heatmap_tile(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "airquality_api", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "current_condition" => self.create_current_condition(input).await,
            "history" => self.create_history(input).await,
            "forecast" => self.create_forecast(input).await,
            "heatmap_tile" => self.create_heatmap_tile(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "airquality_api", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "current_condition" => self.read_current_condition(id).await,
            "history" => self.read_history(id).await,
            "forecast" => self.read_forecast(id).await,
            "heatmap_tile" => self.read_heatmap_tile(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "airquality_api", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "current_condition" => self.update_current_condition(id, input).await,
            "history" => self.update_history(id, input).await,
            "forecast" => self.update_forecast(id, input).await,
            "heatmap_tile" => self.update_heatmap_tile(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "airquality_api", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "current_condition" => self.delete_current_condition(id).await,
            "history" => self.delete_history(id).await,
            "forecast" => self.delete_forecast(id).await,
            "heatmap_tile" => self.delete_heatmap_tile(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "airquality_api", resource_name
            ))),
    ...
    async fn create_current_condition(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a current_condition resource
    async fn read_current_condition(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
    ...
    async fn delete_current_condition(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_heatmap_tile(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a heatmap_tile resource
    async fn read_heatmap_tile(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a heatmap_tile resource
    async fn update_heatmap_tile(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a heatmap_tile resource
    async fn delete_heatmap_tile(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
}
