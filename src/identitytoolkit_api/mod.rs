...
impl<'a> Identitytoolkit_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            ...
            "mfa_enrollment" => self.plan_mfa_enrollment(current_state, desired_input).await,
            "account" => self.plan_account(current_state, desired_input).await,
            "identity_platform" => {
                self.plan_identity_platform(current_state, desired_input)
                    .await
            }
            "inbound_saml_config" => {
                self.plan_inbound_saml_config(current_state, desired_input)
                    .await
            }
            "default_supported_idp" => {
                self.plan_default_supported_idp(current_state, desired_input)
                    .await
            }
            "tenant" => self.plan_tenant(current_state, desired_input).await,
            "identitytoolkit" => {
                self.plan_identitytoolkit(current_state, desired_input)
                    .await
            }
            "mfa_sign_in" => self.plan_mfa_sign_in(current_state, desired_input).await,
            "oauth_idp_config" => {
                self.plan_oauth_idp_config(current_state, desired_input)
                    .await
            }
            "default_supported_idp_config" => {
                self.plan_default_supported_idp_config(current_state, desired_input)
                    .await
            }
            "relyingparty" => self.plan_relyingparty(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "identitytoolkit_api", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            ...
            "mfa_enrollment" => self.create_mfa_enrollment(input).await,
            "account" => self.create_account(input).await,
            "identity_platform" => self.create_identity_platform(input).await,
            "inbound_saml_config" => self.create_inbound_saml_config(input).await,
            "default_supported_idp" => self.create_default_supported_idp(input).await,
            "tenant" => self.create_tenant(input).await,
            "identitytoolkit" => self.create_identitytoolkit(input).await,
            "mfa_sign_in" => self.create_mfa_sign_in(input).await,
            "oauth_idp_config" => self.create_oauth_idp_config(input).await,
            "default_supported_idp_config" => self.create_default_supported_idp_config(input).await,
            "relyingparty" => self.create_relyingparty(input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "identitytoolkit_api", resource_name
            ))),
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            ...
            "mfa_enrollment" => self.read_mfa_enrollment(id).await,
            "account" => self.read_account(id).await,
            "identity_platform" => self.read_identity_platform(id).await,
            "inbound_saml_config" => self.read_inbound_saml_config(id).await,
            "default_supported_idp" => self.read_default_supported_idp(id).await,
            "tenant" => self.read_tenant(id).await,
            "identitytoolkit" => self.read_identitytoolkit(id).await,
            "mfa_sign_in" => self.read_mfa_sign_in(id).await,
            "oauth_idp_config" => self.read_oauth_idp_config(id).await,
            "default_supported_idp_config" => self.read_default_supported_idp_config(id).await,
            "relyingparty" => self.read_relyingparty(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "identitytoolkit_api", resource_name
            ))),
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            ...
            "mfa_enrollment" => self.update_mfa_enrollment(id, input).await,
            "account" => self.update_account(id, input).await,
            "identity_platform" => self.update_identity_platform(id, input).await,
            "inbound_saml_config" => self.update_inbound_saml_config(id, input).await,
            "default_supported_idp" => self.update_default_supported_idp(id, input).await,
            "tenant" => self.update_tenant(id, input).await,
            "identitytoolkit" => self.update_identitytoolkit(id, input).await,
            "mfa_sign_in" => self.update_mfa_sign_in(id, input).await,
            "oauth_idp_config" => self.update_oauth_idp_config(id, input).await,
            "default_supported_idp_config" => {
                self.update_default_supported_idp_config(id, input).await
            }
            "relyingparty" => self.update_relyingparty(id, input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "identitytoolkit_api", resource_name
            ))),
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            ...
            "mfa_enrollment" => self.delete_mfa_enrollment(id).await,
            "account" => self.delete_account(id).await,
            "identity_platform" => self.delete_identity_platform(id).await,
            "inbound_saml_config" => self.delete_inbound_saml_config(id).await,
            "default_supported_idp" => self.delete_default_supported_idp(id).await,
            "tenant" => self.delete_tenant(id).await,
            "identitytoolkit" => self.delete_identitytoolkit(id).await,
            "mfa_sign_in" => self.delete_mfa_sign_in(id).await,
            "oauth_idp_config" => self.delete_oauth_idp_config(id).await,
            "default_supported_idp_config" => self.delete_default_supported_idp_config(id).await,
            "relyingparty" => self.delete_relyingparty(id).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "identitytoolkit_api", resource_name
            ))),
    ...
    async fn create_mfa_enrollment(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a mfa_enrollment resource
    async fn read_mfa_enrollment(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_mfa_enrollment(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_identity_platform(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a identity_platform resource
    async fn read_identity_platform(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_identity_platform(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_inbound_saml_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a inbound_saml_config resource
    async fn read_inbound_saml_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_inbound_saml_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_default_supported_idp(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a default_supported_idp resource
    async fn read_default_supported_idp(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_default_supported_idp(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_identitytoolkit(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a identitytoolkit resource
    async fn read_identitytoolkit(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_identitytoolkit(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_mfa_sign_in(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a mfa_sign_in resource
    async fn read_mfa_sign_in(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a mfa_sign_in resource
    async fn update_mfa_sign_in(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a mfa_sign_in resource
    async fn delete_mfa_sign_in(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_oauth_idp_config(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a oauth_idp_config resource
    async fn read_oauth_idp_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_oauth_idp_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn read_default_supported_idp_config(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_default_supported_idp_config(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_relyingparty(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a relyingparty resource
    async fn read_relyingparty(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a relyingparty resource
    async fn update_relyingparty(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a relyingparty resource
    async fn delete_relyingparty(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
}
