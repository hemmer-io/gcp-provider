...
impl<'a> Dataplex_apiService<'a> {
    ...
    pub async fn plan_resource(
        ...
        match resource_name {
            "data_taxonomie" => self.plan_data_taxonomie(current_state, desired_input).await,
            "change_request" => self.plan_change_request(current_state, desired_input).await,
            "entrie" => self.plan_entrie(current_state, desired_input).await,
            "entry_type" => self.plan_entry_type(current_state, desired_input).await,
            "task" => self.plan_task(current_state, desired_input).await,
            "operation" => self.plan_operation(current_state, desired_input).await,
            "aspect_type" => self.plan_aspect_type(current_state, desired_input).await,
            "governance_rule" => {
                self.plan_governance_rule(current_state, desired_input)
                    .await
            }
            "entitie" => self.plan_entitie(current_state, desired_input).await,
            "asset" => self.plan_asset(current_state, desired_input).await,
            "data_scan" => self.plan_data_scan(current_state, desired_input).await,
            "entry_link_type" => {
                self.plan_entry_link_type(current_state, desired_input)
                    .await
            }
            "content" => self.plan_content(current_state, desired_input).await,
            "glossarie" => self.plan_glossarie(current_state, desired_input).await,
            "action" => self.plan_action(current_state, desired_input).await,
            "metadata_job" => self.plan_metadata_job(current_state, desired_input).await,
            "contentitem" => self.plan_contentitem(current_state, desired_input).await,
            "entry_link" => self.plan_entry_link(current_state, desired_input).await,
            "location" => self.plan_location(current_state, desired_input).await,
            "data_product" => self.plan_data_product(current_state, desired_input).await,
            "entry_group" => self.plan_entry_group(current_state, desired_input).await,
            "zone" => self.plan_zone(current_state, desired_input).await,
            "term" => self.plan_term(current_state, desired_input).await,
            "lake" => self.plan_lake(current_state, desired_input).await,
            ...
            "data_attribute_binding" => {
                self.plan_data_attribute_binding(current_state, desired_input)
                    .await
            }
            ...
            "data_asset" => self.plan_data_asset(current_state, desired_input).await,
            "environment" => self.plan_environment(current_state, desired_input).await,
            _ => Err(hemmer_core::HemmerError::Provider(format!(
                "Unknown resource type: {}.{}",
                "dataplex_api", resource_name
            ))),
        }
    }
    ...
    pub async fn create_resource(
        ...
        match resource_name {
            "data_taxonomie" => self.create_data_taxonomie(input).await,
            "change_request" => self.create_change_request(input).await,
            "entrie" => self.create_entrie(input).await,
            "entry_type" => self.create_entry_type(input).await,
            "task" => self.create_task(input).await,
            "operation" => self.create_operation(input).await,
            "aspect_type" => self.create_aspect_type(input).await,
            "governance_rule" => self.create_governance_rule(input).await,
            "entitie" => self.create_entitie(input).await,
            "asset" => self.create_asset(input).await,
            "data_scan" => self.create_data_scan(input).await,
            "entry_link_type" => self.create_entry_link_type(input).await,
            "content" => self.create_content(input).await,
            "glossarie" => self.create_glossarie(input).await,
            "action" => self.create_action(input).await,
            "metadata_job" => self.create_metadata_job(input).await,
            "contentitem" => self.create_contentitem(input).await,
            "entry_link" => self.create_entry_link(input).await,
            "location" => self.create_location(input).await,
            "data_product" => self.create_data_product(input).await,
            "entry_group" => self.create_entry_group(input).await,
            "zone" => self.create_zone(input).await,
            "term" => self.create_term(input).await,
            "lake" => self.create_lake(input).await,
            ...
            "data_attribute_binding" => self.create_data_attribute_binding(input).await,
            "categorie" => self.create_categorie(input).await,
            "job" => self.create_job(input).await,
            "session" => self.create_session(input).await,
            "data_asset" => self.create_data_asset(input).await,
    ...
    pub async fn read_resource(&self, resource_name: &str, id: &str) -> Result<ResourceOutput> {
        match resource_name {
            "data_taxonomie" => self.read_data_taxonomie(id).await,
            "change_request" => self.read_change_request(id).await,
            "entrie" => self.read_entrie(id).await,
            "entry_type" => self.read_entry_type(id).await,
            "task" => self.read_task(id).await,
            "operation" => self.read_operation(id).await,
            "aspect_type" => self.read_aspect_type(id).await,
            "governance_rule" => self.read_governance_rule(id).await,
            "entitie" => self.read_entitie(id).await,
            "asset" => self.read_asset(id).await,
            "data_scan" => self.read_data_scan(id).await,
            "entry_link_type" => self.read_entry_link_type(id).await,
            "content" => self.read_content(id).await,
            "glossarie" => self.read_glossarie(id).await,
            "action" => self.read_action(id).await,
            "metadata_job" => self.read_metadata_job(id).await,
            "contentitem" => self.read_contentitem(id).await,
            "entry_link" => self.read_entry_link(id).await,
            "location" => self.read_location(id).await,
            "data_product" => self.read_data_product(id).await,
            "entry_group" => self.read_entry_group(id).await,
            "zone" => self.read_zone(id).await,
            "term" => self.read_term(id).await,
            "lake" => self.read_lake(id).await,
            ...
            "data_attribute_binding" => self.read_data_attribute_binding(id).await,
            "categorie" => self.read_categorie(id).await,
            "job" => self.read_job(id).await,
            "session" => self.read_session(id).await,
            "data_asset" => self.read_data_asset(id).await,
    ...
    pub async fn update_resource(
        ...
        match resource_name {
            "data_taxonomie" => self.update_data_taxonomie(id, input).await,
            "change_request" => self.update_change_request(id, input).await,
            "entrie" => self.update_entrie(id, input).await,
            "entry_type" => self.update_entry_type(id, input).await,
            "task" => self.update_task(id, input).await,
            "operation" => self.update_operation(id, input).await,
            "aspect_type" => self.update_aspect_type(id, input).await,
            "governance_rule" => self.update_governance_rule(id, input).await,
            "entitie" => self.update_entitie(id, input).await,
            "asset" => self.update_asset(id, input).await,
            "data_scan" => self.update_data_scan(id, input).await,
            "entry_link_type" => self.update_entry_link_type(id, input).await,
            "content" => self.update_content(id, input).await,
            "glossarie" => self.update_glossarie(id, input).await,
            "action" => self.update_action(id, input).await,
            "metadata_job" => self.update_metadata_job(id, input).await,
            "contentitem" => self.update_contentitem(id, input).await,
            "entry_link" => self.update_entry_link(id, input).await,
            "location" => self.update_location(id, input).await,
            "data_product" => self.update_data_product(id, input).await,
            "entry_group" => self.update_entry_group(id, input).await,
            "zone" => self.update_zone(id, input).await,
            "term" => self.update_term(id, input).await,
            "lake" => self.update_lake(id, input).await,
            ...
            "data_attribute_binding" => self.update_data_attribute_binding(id, input).await,
            ...
            "data_asset" => self.update_data_asset(id, input).await,
    ...
    pub async fn delete_resource(&self, resource_name: &str, id: &str) -> Result<()> {
        match resource_name {
            "data_taxonomie" => self.delete_data_taxonomie(id).await,
            "change_request" => self.delete_change_request(id).await,
            "entrie" => self.delete_entrie(id).await,
            "entry_type" => self.delete_entry_type(id).await,
            "task" => self.delete_task(id).await,
            "operation" => self.delete_operation(id).await,
            "aspect_type" => self.delete_aspect_type(id).await,
            "governance_rule" => self.delete_governance_rule(id).await,
            "entitie" => self.delete_entitie(id).await,
            "asset" => self.delete_asset(id).await,
            "data_scan" => self.delete_data_scan(id).await,
            "entry_link_type" => self.delete_entry_link_type(id).await,
            "content" => self.delete_content(id).await,
            "glossarie" => self.delete_glossarie(id).await,
            "action" => self.delete_action(id).await,
            "metadata_job" => self.delete_metadata_job(id).await,
            "contentitem" => self.delete_contentitem(id).await,
            "entry_link" => self.delete_entry_link(id).await,
            "location" => self.delete_location(id).await,
            "data_product" => self.delete_data_product(id).await,
            "entry_group" => self.delete_entry_group(id).await,
            "zone" => self.delete_zone(id).await,
            "term" => self.delete_term(id).await,
            "lake" => self.delete_lake(id).await,
            ...
            "data_attribute_binding" => self.delete_data_attribute_binding(id).await,
            "categorie" => self.delete_categorie(id).await,
            "job" => self.delete_job(id).await,
            "session" => self.delete_session(id).await,
            "data_asset" => self.delete_data_asset(id).await,
    ...
    async fn create_data_taxonomie(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a data_taxonomie resource
    async fn read_data_taxonomie(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
    ...
    async fn delete_data_taxonomie(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_change_request(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a change_request resource
    async fn read_change_request(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_change_request(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_entry_type(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a entry_type resource
    async fn read_entry_type(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a entry_type resource
    async fn update_entry_type(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn create_aspect_type(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a aspect_type resource
    async fn read_aspect_type(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a aspect_type resource
    async fn update_aspect_type(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a aspect_type resource
    async fn delete_aspect_type(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_governance_rule(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a governance_rule resource
    async fn read_governance_rule(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_governance_rule(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_data_scan(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a data_scan resource
    async fn read_data_scan(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a data_scan resource
    async fn update_data_scan(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a data_scan resource
    async fn delete_data_scan(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_entry_link_type(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a entry_link_type resource
    async fn read_entry_link_type(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_entry_link_type(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_content(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a content resource
    async fn read_content(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a content resource
    async fn update_content(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn create_glossarie(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a glossarie resource
    async fn read_glossarie(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a glossarie resource
    async fn update_glossarie(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a glossarie resource
    async fn delete_glossarie(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_metadata_job(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a metadata_job resource
    async fn read_metadata_job(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a metadata_job resource
    async fn update_metadata_job(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a metadata_job resource
    async fn delete_metadata_job(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_contentitem(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a contentitem resource
    async fn read_contentitem(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a contentitem resource
    async fn update_contentitem(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a contentitem resource
    async fn delete_contentitem(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_entry_link(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a entry_link resource
    async fn read_entry_link(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a entry_link resource
    async fn update_entry_link(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a entry_link resource
    async fn delete_entry_link(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_data_product(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a data_product resource
    async fn read_data_product(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a data_product resource
    async fn update_data_product(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a data_product resource
    async fn delete_data_product(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_term(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a term resource
    async fn read_term(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a term resource
    async fn update_term(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn create_lake(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a lake resource
    async fn read_lake(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a lake resource
    async fn update_lake(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a lake resource
    async fn delete_lake(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_data_attribute_binding(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a data_attribute_binding resource
    async fn read_data_attribute_binding(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }
    ...
    async fn delete_data_attribute_binding(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
    ...
    async fn create_data_asset(&self, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id("placeholder-id"))
    }

    /// Read a data_asset resource
    async fn read_data_asset(&self, id: &str) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Update a data_asset resource
    async fn update_data_asset(&self, id: &str, input: ResourceInput) -> Result<ResourceOutput> {
        // TODO: Implement Gcp SDK calls
        Ok(ResourceOutput::new().with_id(id))
    }

    /// Delete a data_asset resource
    async fn delete_data_asset(&self, id: &str) -> Result<()> {
        // TODO: Implement Gcp SDK calls
        Ok(())
    }
