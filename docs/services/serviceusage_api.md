...
| `check_if_service_has_usage` | String |  | Defines the behavior for checking service usage when disabling a service. |
...
service = provider.serviceusage_api.Service {
    name = "value"  # Name of the consumer and service to disable the service on. The enable and disable methods currently only support projects. An example name would be: `projects/123/services/serviceusage.googleapis.com` where `123` is the project number.
}
...
# Create service
service = provider.serviceusage_api.Service {
    name = "value"  # Name of the consumer and service to disable the service on. The enable and disable methods currently only support projects. An example name would be: `projects/123/services/serviceusage.googleapis.com` where `123` is the project number (not project ID).
}
...
service_0 = provider.serviceusage_api.Service {
    name = "value-0"
}
service_1 = provider.serviceusage_api.Service {
    name = "value-1"
}
service_2 = provider.serviceusage_api.Service {
    name = "value-2"
}
```
