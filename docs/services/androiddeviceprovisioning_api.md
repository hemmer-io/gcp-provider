...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `device` | String |  | Required. The device to remove the configuration from. There are custom validations in RemoveConfigurationRequestValidator |
...
device = provider.androiddeviceprovisioning_api.Device {
    parent = "value"  # Required. The customer managing the device in the format `customers/[CUSTOMER_ID]`.
}
...
| Output | Type | Description |
|--------|------|-------------|
| `customers` | Vec<String> | The customer accounts the calling user is a member of. |
| `next_page_token` | String | A token used to access the next page of results. Omitted if no further results are available. |
