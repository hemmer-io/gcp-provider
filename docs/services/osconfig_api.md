...
| `rollout` | String |  | Rollout strategy of the patch job. |
| `dry_run` | bool |  | If this patch is a dry-run only, instances are contacted but will do nothing. |
| `duration` | String |  | Duration of the patch job. After the duration ends, the patch job times out. |
| `instance_filter` | String |  | Required. Instances to patch, either explicitly or filtered by some criteria such as zone or labels. |
| `patch_config` | String |  | Patch configuration being applied. If omitted, instances are patched using the default configurations. |
...
patch_job = provider.osconfig_api.Patch_job {
    parent = "value"  # Required. The project in which to run this patch in the form `projects/*`
}
...
operation_0 = provider.osconfig_api.Operation {
    name = "value-0"
}
operation_1 = provider.osconfig_api.Operation {
    name = "value-1"
}
operation_2 = provider.osconfig_api.Operation {
    name = "value-2"
}
```
