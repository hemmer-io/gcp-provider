...
The gameservices_api service provides access to 6 resource types:

- [Location](#location) [R]
- [Operation](#operation) [CRD]
- [Game_server_deployment](#game_server_deployment) [CR]
- [Location](#location) [R]
- [Operation](#operation) [CRD]
- [Game_server_deployment](#game_server_deployment) [CR]
...
# Create operation
operation = provider.gameservices_api.Operation {
...
| `rules` | Vec<String> | If more than one rule is specified, the rules are applied in the following manner: - All matching LOG rules are always applied. - If any DENY/DENY_WITH_LOG rule matches, permission is denied. Logging will be applied if one or more matching rule requires logging. - Otherwise, if any ALLOW/ALLOW_WITH_LOG rule matches, permission is granted. Logging will be applied if one or more matching rule requires logging. - Otherwise, if no rule applies, permission is denied. |
...
# Create game_server_deployment
game_server_deployment = provider.gameservices_api.Game_server_deployment {
...
# Access game_server_deployment outputs
game_server_deployment_id = game_server_deployment.id
game_server_deployment_audit_configs = game_server_deployment.audit_configs
game_server_deployment_etag = game_server_deployment.etag
game_server_deployment_version = game_server_deployment.version
game_server_deployment_bindings = game_server_deployment.bindings
game_server_deployment_rules = game_server_deployment.rules
```
...
location_0 = provider.gameservices_api.Location {
}
location_1 = provider.gameservices_api.Location {
}
location_2 = provider.gameservices_api.Location {
}
```
...
if environment == "production":
    location = provider.gameservices_api.Location {
    }
```
...
- [GCP Gameservices_api Documentation](https://cloud.google.com/gameservices_api/docs)
